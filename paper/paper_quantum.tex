...
\caption{\textbf{CQEC maintains unit fidelity at noise levels where Steane and surface codes degrade monotonically.}
...
We verify all previously assumed properties directly: covariance $\|[U, H_\mathrm{total}]\| = 0$ exactly, linearity residual $< 2 \times 10^{-16}$, Choi-matrix positivity, and the catalyst marginal obtained by partial trace of the propagated joint state (never copied).
...
The unit test suite (\texttt{pytest tests/}, 44~tests) validates each packaged module against the corresponding quantitative claims.
